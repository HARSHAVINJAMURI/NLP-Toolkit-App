...
# ------------------------------
# NLTK setup: pre-load data for Hugging Face
# ------------------------------
nltk_data_packages = [
    "punkt",
    "stopwords",
...
# Add local nltk_data folder path
...
with col1:
    ...
    st.subheader("Text processing & analysis using NLTK")
with col2:
...
with st.sidebar:
    st.header("Input")
    src = st.radio("Source", ["Paste text", "Upload file"])
    txt = ""
    if src == "Paste text":
    ...
    stop_lang = st.selectbox("Stopwords language", ["english","none"])
    stem_choice = st.selectbox("Stemming", ["None","Porter","Lancaster","Snowball"])
...
if run:
    if not txt.strip():
        ...
        # Downloads
        st.download_button("Download processed tokens (.txt)", "\n".join(tokens_processed),
                           file_name="tokens_processed.txt", mime="text/plain")
        st.download_button("Download original tokens (.txt)", "\n".join(tokens_original),
